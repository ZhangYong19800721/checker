...
class LOADER(object):
    ...
    def __init__(self, dataset, minibatch_size=100):
        ...
        self.minibatch_num =  max(pos_len // int(self.minibatch_size * 0.8), neg_len // int(self.minibatch_size * 0.2))  # batch的数量
    ...
    def __getitem__(self, idx):
        pos_len, neg_len = self.dataset.getLen()
        select_pos_id = random.choices(range(pos_len), k=int(self.minibatch_size * 0.8))
        select_neg_id = random.choices(range(neg_len), k=int(self.minibatch_size * 0.2))
        minibatch = []
